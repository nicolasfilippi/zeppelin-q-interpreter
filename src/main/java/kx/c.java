...
protected c(){};
public static class Month implements Comparable<Month>{public int i;public Month(int x){i=x;}@Override
public String toString(){int m=i+24000,y=m/12;return i==ni?"":i2(y/100)+i2(y%100)+"-"+i2(1+m%12);}@Override
public boolean equals(final Object o){return(o instanceof Month)?((Month)o).i==i:false;}@Override
public int hashCode(){return i;}@Override
public int compareTo(Month m){return i-m.i;}}
public static class Minute implements Comparable<Minute>{public int i;public Minute(int x){i=x;}@Override
public String toString(){return i==ni?"":i2(i/60)+":"+i2(i%60);}
@Override
public boolean equals(final Object o){return(o instanceof Minute)?((Minute)o).i==i:false;}@Override
public int hashCode(){return i;}@Override
public int compareTo(Minute m){return i-m.i;}}
public static class Second implements Comparable<Second>{public int i;public Second(int x){i=x;}@Override
public String toString(){return i==ni?"":new Minute(i/60).toString()+':'+i2(i%60);}@Override
public boolean equals(final Object o){return(o instanceof Second)?((Second)o).i==i:false;}@Override
public int hashCode(){return i;}@Override
public int compareTo(Second s){return i-s.i;}}
...
@Override
public String toString(){if(j==nj)return "";String s=j<0?"-":"";long jj=j<0?-j:j;int d=((int)(jj/86400000000000L));if(d!=0)s+=d+"D";return s+i2((int)((jj%86400000000000L)/3600000000000L))+":"+i2((int)((jj%3600000000000L)/60000000000L))+":"+i2((int)((jj%60000000000L)/1000000000L))+"."+i9((int)(jj%1000000000L));}@Override
public int compareTo(Timespan t){return j>t.j?1:j<t.j?-1:0;}
@Override
public boolean equals(final Object o){return(o instanceof Timespan)?((Timespan)o).j==j:false;}@Override
public int hashCode(){return(int)(j^(j>>>32));}}
...
private void u(){int n=0,r=0,f=0,s=8,p=s;short i=0;j=0;byte[]dst=new byte[ri()];int d=j;int[]aa=new int[256];while(s<dst.length){if(i==0){f=0xff&b[d++];i=1;}if((f&i)!=0){r=aa[0xff&b[d++]];dst[s++]=dst[r++];dst[s++]=dst[r++];n=0xff&b[d++];for(int m=0;m<n;m++)dst[s+m]=dst[r+m];}else dst[s++]=b[d++];while(p<s-1)aa[(0xff&dst[p])^(0xff&dst[p+1])]=p++;if((f&i)!=0)p=s+=n;i*=2;if(i==256)i=0;}b=dst;j=8;}
...
// XXX: NF Add a close on the stream
...
protected void w(int i,Object x)throws IOException{int n=nx(x)+8;synchronized(o){B=new byte[n];B[0]=0;B[1]=(byte)i;J=4;w(n);w(x);if(zip&&J>2000&&!l)z();o.write(B,0,J);o.flush();}}
...
/*public static void main(String[]args){ // example client for query/response
  ...
  try{
    ...
    //c.setEncoding("UTF-8");
    //O("Unicode "+c.k("{`$x}","Ranby BjÃÂ¶rklund AB".toCharArray()));
